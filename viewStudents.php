...
<body>
  ...
  <div id="wrapper">
    ...
    <div id="page-wrapper">
      ...
      <div class="row">
        <div class="col-lg-12">
          <h1>VIEW <small>Students</small></h1>
          <ol class="breadcrumb">
            <li class="active"><i class="fa fa-dashboard"></i> View Students</li>
          </ol>
  ...
  <div class="row">
      ...
      <div class="panel panel-primary">
        <div class="panel-heading">
          <h3 class="panel-title"><i class="fa fa-bar-chart-o"></i> Students : '.date("M d Y").'</h3>
        </div>
        <div class="panel-body">
          
          <form name="viewStudents" action="viewStudents.php" method="GET">
          <div class="row">
            <div class="col-md-2">Course</div>
            <div class="col-md-2">
          
            <select class="form-control input-sm" name="selectedCourse"> ';

                 // $q2 = "SELECT * FROM teacher WHERE `email` = '$email'";
           

                  //$query = mysql_query($q2) or die("error selecting from teacher");
      
                  //while($row = mysql_fetch_array($query)){
                      $coursesAssigned = explode(",", $coursesAssigned);
              ...
              <div class="col-md-2"><button name="submit" onclick="viewStudents.submit();" class="btn btn-primary">Show</a></div>
          </div>

          </form>
          <h4><strong><u>';
          if(isset($_GET['submit'])){
            echo $_GET['selectedCourse'];
          }
          echo '</u></strong></h4>
          <table class="table table-hover" style="font-size:12px;">
              ...
              $selectedCourse = $_GET['selectedCourse'];
               ...
               $q2 = "SELECT * FROM `students` INNER JOIN `users` ON students.email = users.email WHERE `coursesRegistered` LIKE '%$selectedCourse%'";
             ...
             //$row = mysql_fetch_array($results) or die("error fetching" . mysql_error() );
              //echo $row['email'];
              while($row = mysql_fetch_array($results)){

                echo '
                        <tr>
                          <td><span class="glyphicon glyphicon-user"></span> '.$row['regNo'].'</td>
                          <td>'.$row['fname'].'</td>
                          <td>'.$row['lname'].'</td>
